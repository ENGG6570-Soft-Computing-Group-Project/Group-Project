...
class AminKNeuronMethod:
    def __init__(self,target_list,robot_list):
        ...
        self.m = len(target_list)
        self.k = len(robot_list)
        self.wx = [0]*self.k
        self.wy = [0]*self.k
        self.Dmin = 0
        ...
        self.Dmin = math.sqrt(currentDmin)
        print("the min distance  is ", self.Dmin)

        # This is the path for all the robots
        self.path = []
    ...
    def oneStepWhenKequalM(self, t=0, belta = 0.5):
        ...
        for i in inputSeq:
            ...
            # 4) learning for all the output layer neurons
            for l in range(self.k):
                # if a robot arrived to target, it will not learn anymore
                if self.arrived2Task[l] == 1:
                    continue
                if dij[l] < 0.5*self.Dmin:
                    # if a robot can arrived to a task, then, assign the robot to the task, and do not move it anymore
                    self.wx[l] = x
                    self.wy[l] = y
                    print(" the distance between i, j:", i, l, " is ", dij[l], ", this is smaller than ", self.Dmin)
